	...
	public static function generatePage(&$tpl, &$session, &$account, &$perso)
	{
		...
		while($item = $casier->getInventaire($i++))
		{
			if(isset($_POST['qte_' . $item->getInvId()]))
			{
				...
				if($qte>0)
				{
					...
					$itemsList .= (($e++>0) ? ', ' : '') . $qte . ' [i]' . $item->getNom() . '[/i]';
		...
		while( $tmp = $perso->getLieu()->getPerso($perso, $i++))
			if($tmp->getId() != $perso->getId())
				$arrFrom[$e++] = $tmp->getId();
		Member_He::add($perso->getId(), $arrFrom, 'casier', "Quelqu'un semble prendre un objet ddans le casier \'" . $casier->getNom() . "\'.", HE_AUCUN, HE_UNIQUEMENT_MOI);
