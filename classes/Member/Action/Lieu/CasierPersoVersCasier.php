	...
	public static function generatePage(&$tpl, &$session, &$account, &$perso)
	{
		...
		while($item = $perso->getInventaire($i++))
		{
			if(isset($_POST['qte_' . $item->getInvId()]) && $_POST['qte_' . $item->getInvId()]>0){
				...
				$msg .= $_POST['qte_' . $item->getInvId()] .' [i]'. $item->getNom().'[/i]';
		...
		while( $tmp = $perso->getLieu()->getPerso($perso, $i++))
			if($tmp->getId() != $perso->getId())
				$arrFrom[$e++] = $tmp->getId();
		Member_He::add($perso->getId(), $arrFrom, 'casier', "Quelqu'un semble déposer un objet dans le casier \'" . $casier->getNom() . "\'.", HE_AUCUN, HE_UNIQUEMENT_MOI);
