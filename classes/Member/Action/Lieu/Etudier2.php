...
class Member_Action_Lieu_Etudier2
{
	public static function generatePage(&$tpl, &$session, &$account, &$perso)
	{
		...
		$perso->getLieu()->addBoutiqueHistorique('vente', $coutCashTotal, 'temps d\'étude', 0, 0);
                ...
                //Chercher le nom de la competence
                $query = 'SELECT nom'
				. ' FROM `' . DB_PREFIX . 'competence`'
				. ' WHERE	`abbr`=:comp'
				. ' LIMIT 1;';
		$prep = $db->prepare($query);
                $prep->bindValue(':comp',		strtolower($_POST['comp']),					PDO::PARAM_STR);
		$prep->execute($db, __FILE__, __LINE__);
		$arr2 = $prep->fetch();
		$prep->closeCursor();
		$prep = NULL;
		...
		while( $tmp = $perso->getLieu()->getPerso($perso, $i++))
			if($tmp->getId() != $perso->getId())
                ...
                if (preg_match("#^[aeiouy]#", strtolower($arr2['nom'])))
		Member_He::add($perso->getId(), $arrFrom, 'etude', "Quelqu'un semble en plein apprentissage d'" . strtolower($arr2['nom']), HE_AUCUN, HE_UNIQUEMENT_MOI);
		else
                Member_He::add($perso->getId(), $arrFrom, 'etude', "Quelqu'un semble en plein apprentissage de " . strtolower($arr2['nom']), HE_AUCUN, HE_UNIQUEMENT_MOI);
