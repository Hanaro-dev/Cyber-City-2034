...
class Member_Action_Lieu_Soigner2
{
	public static function generatePage(&$tpl, &$session, &$account, &$perso)
	{
		...
		if($pa <= 0)
			return fctErrorMSG('La valeur des PA doit être suppérieure à 0.', $errorUrl);
		...
		$reussiteChrg =  $perso->getChancesReussite("MEDS");
		$gainPV = 0;
		$materielUtilise = 0;
		...
		for($i=0;
			...
			$materielUtilise = round($i / $facteurResistance);
			if ($materielUtilise == 0)
				$materielUtilise = 1;
		...
		$msg = 'Des soins sont prodigués et permettent de guérir ' . $gainPV . 'PV. ';
		$msg .= $perso->setComp(array('MEDS' => (rand(1,2) * (int) ($pa / 5))));
		
		Member_He::add($perso->getId(), $blesse->getId(), 'soin', $msg);
		...
		Member_He::add(array($perso->getId(), $blesse->getId()), $arrPersoLieu, 'soin', "Vous voyez quelqu'un utiliser le matériel médical.", HE_AUCUN, HE_UNIQUEMENT_MOI);
