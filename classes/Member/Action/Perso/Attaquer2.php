...
class Member_Action_Perso_Attaquer2
{
	public static function generatePage(&$tpl, &$session, &$account, &$perso)
	{
		...
		//Vérifier si l'on a assez de balles pour attaquer
		if (get_class($perso->getArme()) == 'Member_ItemArmeFeu' && $perso->getArme()->getMunition() < $_POST['tours'])
			return fctErrorMSG('Vous n\'avez pas assez de balles dans votre chargeur.', $errorUrl);
		...
		$persoRate = 0; //Nombre de raté pour les armes à feu
		$victimeRate = 0;
		$testfuite = 0; //Variable de test pour la fuite
		$coutFuite = 0;
		...
		{
			if (!strcmp($_POST['zones'],'Tete'))
				$loc = 'Tête';
			else
				$loc = $_POST['zones'];
			
			$msgPerso .= " ciblée (Zone visée: " . $loc . "):";
		}
		else
		...
		do //Boucle des tours d'attaques
		{
			...
			switch(get_class($perso->getArme()))
			{
				...
				case 'Member_ItemArmeBlanche':
					...
					break;
                case 'Member_ItemArmeCac':
                    $riposte = Member_Action_Perso_Attaquer2ArmeCac::attaquer($perso, $victime, $porteeSelectionnee, $msgPerso, $msgVictime, $msgLieu, $tour, $victimeEsq, false);
					break;
				case 'Member_ItemArmeFeu':
					$riposte = Member_Action_Perso_Attaquer2ArmeFeu::attaquer($perso, $victime, $porteeSelectionnee, $msgPerso, $msgVictime, $msgLieu, $tour, $victimeEsq, false, $persoRate);
					break;
				...
				else //La riposte a lieu
				{
					...
					switch(get_class($victime->getArme()))
					{
						...
						case 'Member_ItemArmeBlanche':
							...
							break;
						case 'Member_ItemArmeCac':
							$riposte = Member_Action_Perso_Attaquer2ArmeCac::attaquer($victime, $perso, $porteeSelectionnee, $msgPerso, $msgVictime, $msgLieu, $tour, $victimeEsq, true);
							break;
						case 'Member_ItemArmeFeu':
							$riposte = Member_Action_Perso_Attaquer2ArmeFeu::attaquer($victime, $perso, $porteeSelectionnee, $msgPerso, $msgVictime, $msgLieu, $tour, $persoEsq, true, $victimeRate);
							break;
						//case 'Member_ItemArmeExplosive':
			...
			$statid = $perso->convStatCodeToId("PER");
			
			die("toto");
			
				$reussite_arret = 85 + $perso->getStatRealLevel($statid)*3;
			...
			*/
			...
			if ($riposte && $victime->isAutonome() && $victime->getPa()>0 && $victime->getReaction()=='fuir' && $victimeEsq + $persoRate + $testfuite != $tour)
			{
				$testfuite++;
				$msgPerso	.= "\n\nLa victime tente de fuir mais vous ne la laissez pas faire.";
				$msgVictime	.= "\n\nVous tentez de fuir mais votre agresseur vous rattrape.";
				$msgLieu	.= "\n\nLa victime tente de fuir mais n'y arrive pas.";
			}
			else if ($riposte && $victime->isAutonome() && $victime->getPa()>0 && $victime->getReaction()=='fuir')
			{
				//Lister tout les lieux en libre accès accessibles de l'emplacement actuel
				$i=0;
				if (isset($lieuFuite)) unset($lieuFuite);
				
				while($lien = $perso->getLieu()->getLink($i++))
					if($lien->getProtection()=='0' && $lien->getCout() <= 3 && $lien->getCout() < $victime->getPa())
						$lieuFuite[] = $lien;
				...
				if($e>0 && $r < 2)
				{
					...
					$coutFuite = $lieuFuite[$choix]->getCout();
					$attStop = true;
				}
				else
				{
			...
			else if ($riposte && $victime->isConscient() && $victime->getReaction()=='fuir')
			{
					$msgPerso	.= "\n\nVotre victime a l'air de vouloir fuir mais est en trop mauvaise condition.";
					$msgVictime	.= "\n\nVous voudriez fuir mais vous êtes en trop mauvaise condition.";
					$msgLieu	.= "\n\nLa victime a l'air de vouloir fuir mais son état ne lui permet pas.";
			}
		...
		//Recalcul des Pa effectivement dépensés
		$cout_pa = Member_Action_Perso_Attaquer::coutPaTotal($perso, $tour);
		if ($_POST['type_att'] == 'cible')	$cout_pa+=15;
		if (isset($_POST['furtif']))		$cout_pa+=15;
