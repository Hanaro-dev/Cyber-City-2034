										...
										$estUneRiposte=false)
	{
		...
		//Effectuer l'attaque attaques
			...
			else //L'attaque réussit, calculer les dommages à la victime
			{
					...
					$forceArme = $persoArme->getForce();
					$baseForce = 8;
					$diffForce = $perso->getCompRealLevel($armbId) - $baseForce;
					if($diffForce >= 0 && $perso->getRetenircoup() == 0)
					{
					   $forceArme = 15 + $diffForce * 3;
					}
					$degats += $forceArme * (rand(1,100)/100);
					...
					if(isset($victimeDefense))
					{
						...
						if($perso->getCompRealLevel($armbId) < $baseForce) //Le perso peut se blesser si ARMC < 8
						{
