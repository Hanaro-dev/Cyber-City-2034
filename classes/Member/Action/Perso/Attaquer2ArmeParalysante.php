...
*/
class Member_Action_Perso_Attaquer2ArmeParalysante extends Member_Action_Perso_Attaquer2
{
										...
										$estUneRiposte=false)
	{
		...
		else	//L'attaque réussit, calculer les dommages à la victime
		{
			...
			while( $item = $victime->getInventaire($i++))
			{
				...
				{
					if($item->isEquip() && $item->getResistance() != 0)
					{
						$victimeDefense = $item;
						break;
					}
			...
			if(isset($victimeDefense))
			{
				...
				//$degats = $victimeDefense->absorbDamage($degats, $msgPerso, $msgVictime, $estUneRiposte);
			...
			//Mettre à jour les PA de la victime
			$victime->changePa('-', $degats);
			
			
			if (!$estUneRiposte)
			{
				$msgPerso	.= "\nVous arrivez à blesser votre victime {$txtLoc} de {$degats} PA.";
				$msgVictime	.= "\nVotre aggresseur arrive à vous blesser {$txtLoc} de {$degats} PA.";
				$msgLieu	.= "\nL'aggresseur arrive à porter un coup {$txtLoc}.";
			}
			else
			{
				$msgPerso	.= "\nVotre victime arrive à vous infliger {$degats} PA de dégat {$txtLoc}.";
				$msgVictime	.= "\nVous arrivez à blesser votre aggresseur {$txtLoc} de {$degats} PA.";
