...
class Member_Action_Perso_Soigner2
{
	public static function generatePage(&$tpl, &$session, &$account, &$perso)
	{
		...
		$reussiteScrs =  $perso->getChancesReussite("MEDS");
		$compteurPV = 0;
		$pafinal = 0;
		...
		if($itemId != 0) //Si on soigne avec une trousse
		{
			...
			for($i=0;
				($i<$pa)
					...
					&& (round( $i / $facteurResistance) < $trousse->getResistance());
				$i++)
			{
		...
		if($itemId != 0)
		{
			$resistanceRetiree = round( $i / $facteurResistance);
			if ($resistanceRetiree == 0)
				$resistanceRetiree = 1;
		...
		$perso->setComp(array('MEDS' => rand(1,3) ));
		$perso->setStat(array('AGI' => '-01', 'FOR' => '-01', 'DEX' => '+02', 'PER' => '-01', 'INT' => '+01'));
		
		if($perso->getId() != $blesse->getId())
		{
			$msgPerso = 'Vous utilisez ' . $pafinal . ' PA pour dispenser des soins à raison de ' . $compteurPV . ' PV redonnés.';
			$msgBlesse = 'Une personne vous prodigue des soins à raison de ' . $compteurPV . 'PV.';
			$msgAutre = 'Vous voyez quelqu\'un dispenser des soins à un blessé.';
			
			$msgPerso .= "\n\n" . $perso->setStat(array('AGI' => '-01', 'FOR' => '-01', 'DEX' => '+02', 'PER' => '-01', 'INT' => '+01'));
			$msgPerso .= "\n" . $perso->setComp(array('MEDS' => rand(1,3) ));
			
			//Envoyer le message aux 2 personnes impliqués
			Member_He::add($perso->getId(), $blesse->getId(), 'soigner', $msgPerso, HE_TOUS, HE_AUCUN);
			Member_He::add($perso->getId(), $blesse->getId(), 'soigner', $msgBlesse, HE_AUCUN, HE_TOUS);
			
			//Envoyer le message à tout les gens présent sur le lieu
			$i=0;
			$arrPersoLieu = array();
			while( $tmp = $perso->getLieu()->getPerso($perso, $i++))
				if($tmp->getId() != $perso->getId() && $tmp->getId() != $blesse->getId())
					$arrPersoLieu[count($arrPersoLieu)] = $tmp->getId();
			
			if(count($arrPersoLieu)>0)
				Member_He::add(array($perso->getId(), $blesse->getId()), $arrPersoLieu, 'soigner', $msgAutre, HE_AUCUN, HE_TOUS);
		}
		else
		{
			$msgPerso = 'Vous utilisez ' . $pafinal . ' PA pour soigner vos blessures à raison de ' . $compteurPV . ' PV.';
			$msgAutre = 'Vous voyez quelqu\'un soigner ses blessures.';
			
			$msgPerso .= "\n\n" . $perso->setStat(array('AGI' => '-01', 'FOR' => '-01', 'DEX' => '+02', 'PER' => '-01', 'INT' => '+01'));
			$msgPerso .= "\n" . $perso->setComp(array('MEDS' => rand(1,3) ));
			
			//Envoyer le message système à la personne impliquée
			Member_He::add('System', $perso->getId(), 'soigner', $msgPerso);
			...
			if(count($arrPersoLieu)>0)
				Member_He::add($perso->getId(), $arrPersoLieu, 'soigner', $msgAutre, HE_AUCUN, HE_TOUS);
