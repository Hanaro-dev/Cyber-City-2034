...
abstract class Member_Item
{
	...
	/** Retourne l'état approximatif d'un item.
	 * @return string
	 */
	public function getEtat() {
		//$res = $item->getResistance();
		//$resMax = $item->getResistanceMax();
		$resPourcent = $this->getResistance() * 100 / $this->getResistanceMax();
		$resEtat = "";
		
		if($resPourcent > 70) { $resEtat = "peu servi"; }
		else if($resPourcent > 30) { $resEtat = "<span class='txtStyle_risque'>usagé</span>"; }
		else if($resPourcent > 0) { $resEtat = "<span class='txtStyle_critique'>détérioré</span>"; }
	 ...
	 * @return string
	 */
	public function getTypeItem()			{ return $this->type; }
	
	/** Retourne le sous-type d'un item.
	 * @return string
	 */
	public function getSousTypeItem()			{ return $this->soustype; }
