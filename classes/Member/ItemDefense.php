	...
	public function absorbDamage($degats, &$msgPerso, &$msgVictime, $txtLoc='', $estUneRiposte=false)
	{
		...
		else //Les dégats sont inférieur au seuil de l'armure
		{
			...
			{				//L'armure est morte
				...
				if($estUneRiposte==true)
				{
					$msgPerso	.= "\nVotre victime frappe sur votre armure {$txtLoc} qui qui la rend hors d'état ({$amorti} PV)";
					$msgVictime	.= "\nVous frappez sur l'armure de votre agresseur {$txtLoc} que vous rendez hors d'état ({$amorti} PV)";
				}
				else
				{
					$msgPerso	.= "\nVous frappez sur une armure {$txtLoc} que vous rendez hors d'état ({$amorti} PV)";
			...
			else	//L'armure encaisse, absorbe tout les dégats et résiste
			{
				...
				if($estUneRiposte==true)
				{
					$msgPerso	.= "\nVotre victime frappe sur votre armure {$txtLoc} qui absorbe les dégats ({$amorti} PV)";
					$msgVictime	.= "\nVous frappez sur l'armure de votre agresseur {$txtLoc} qui absorbe les dégats ({$amorti} PV)";
				}
				else
				{
