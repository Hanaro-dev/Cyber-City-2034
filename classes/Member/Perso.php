...
class Member_Perso
{
	...
	function __construct(&$arr=array())
	{
		...
		$this->retenircoup	= (string)$arr['retenircoup'];
		...
		if ( $this->getLieu()->getNomTech() != INNACTIVITE_VOLUNTARY_LOCATION 
			...
			// GÉNÉRER le menu en rapport avec le LIEU
				...
				foreach($arrLieuMenu as &$arr)
					$menu[] = array('url' => 'Lieu_' . $arr['url'] . '&modid=' . $arr['id'], 'txt' => $arr['caption']);
	...
	*/
	...
	{
            if ($this->isAutonome() && $this->getPa() != 0 )
		return $this->esquive;
            else
                return false;
	}
	...
	/**
	 * Retourne le mode de retenue des coups.
	 *
	 * Légende:
	 * - '0' = non
	 * - '1' = oui
	 ...
	 * @return string
	 */
	public function getRetenircoup()
	{
		return $this->retenircoup;
	}
	...
	//Fonction qui modifie les PVMax (utilisation uniquement avec les drogues
	public function changePvMax($signe, $nbrPv)
	{
		$dbMgr = DbManager::getInstance(); //Instancier le gestionnaire
		$db = $dbMgr->getConn('game'); //Demander la connexion existante
		
                if ($signe=='+')
                    $nbrPv = $this->pvMax + $nbrPv;
                else
                    $nbrPv = $this->pvMax - $nbrPv;
		...
		$query = 'UPDATE ' . DB_PREFIX . 'perso '
					...
					. ' LIMIT 1;';
		$prep = $db->prepare($query);
		$prep->bindValue(':pvmax',			$nbrPv,		PDO::PARAM_INT);
		$prep->bindValue(':persoId',	$this->id,	PDO::PARAM_INT);
		$prep->execute($db, __FILE__,__LINE__);
		$prep->closeCursor();
		$prep = NULL;
        ...
        //Fonction qui modifie les PAMax (utilisation uniquement avec les drogues
	public function changePaMax($signe, $nbrPa)
	{
		$dbMgr = DbManager::getInstance(); //Instancier le gestionnaire
		$db = $dbMgr->getConn('game'); //Demander la connexion existante
		
                if ($signe=='+')
                    $nbrPa = $this->paMax + $nbrPa;
                else
                    $nbrPa = $this->paMax - $nbrPa;
		...
		$query = 'UPDATE ' . DB_PREFIX . 'perso '
					...
					. ' LIMIT 1;';
		$prep = $db->prepare($query);
		$prep->bindValue(':pamax',			$nbrPa,		PDO::PARAM_INT);
		$prep->bindValue(':persoId',	$this->id,	PDO::PARAM_INT);
		$prep->execute($db, __FILE__,__LINE__);
		$prep->closeCursor();
		$prep = NULL;
	...
	public static function convCompLevelToXp ( $lvl ) 
	{
		...
		switch($lvl)
		{
			...
			case 3:		return 110;		break;
			case 4:		return 180;		break;
			case 5:		return 275;		break;
			case 6:		return 405;		break;
			case 7:		return 605;		break;
			case 8:		return 875;	break;
			case 9:		return 1215;	break;
			case 10:	return 1615;	break;
			case 11:	return 2065;	break;
			case 12:	return 2565;	break;
		}
	}
