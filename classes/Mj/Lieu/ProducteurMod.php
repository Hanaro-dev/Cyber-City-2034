...
class Mj_Lieu_ProducteurMod
{
	...
	private static function save()
	{
		...
		/*if (!isset($_POST['lieuMenuId']))
			return fctErrorMSG('Le module de production doit être associé à une action de lieu.', '?mj=Lieu_ProducteurAdd',null,false);*/
		...
		if (!is_numeric($_POST['pa_cash_ratio']) && !is_float($_POST['pa_cash_ratio']))
			return fctErrorMSG('Le ratio PA/Cash doit être un nombre décimal.', '?mj=Lieu_ProducteurMod',null,false);
		
		if (!is_numeric($_POST['cash']))
			return fctErrorMSG('Le cash doit être un nombre entier.', '?mj=Lieu_ProducteurMod',null,false);
		
		if (!is_numeric($_POST['comp_lvl']))
			return fctErrorMSG('Le niveau de competence minimum doit être un entier.', '?mj=Lieu_ProducteurMod',null,false);
                
                if (!is_numeric($_POST['comp_xp']))
			return fctErrorMSG('L\'xp gagnée doit être un entier.', '?mj=Lieu_ProducteurMod',null,false);
		...
		$query = 'UPDATE ' . DB_PREFIX . 'producteur'
				. ' SET'
					...
					. ' total_pa	= :total_pa,'
					. ' pa_needed	= :pa_needed,'
                                        . ' comp_requise= :comp_requise,'
                                        . ' comp_lvl	= :comp_lvl,'
                                        . ' comp_xp	= :comp_xp,'
					. ' lieuMenuId		= :lieuMenuId'
				. ' WHERE id=:id;';
		$prep = $db->prepare($query);
		$prep->bindValue(':lieuId',		$_POST['lieuId'],			PDO::PARAM_INT);
		...
		$prep->bindValue(':lieuMenuId',		$_POST['lieuMenuId'],			PDO::PARAM_INT);
		$prep->bindValue(':id',			$_POST['id'],				PDO::PARAM_INT);
		$prep->execute($db, __FILE__, __LINE__);
		$prep->closeCursor();
		$prep = NULL;
