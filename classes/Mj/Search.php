		...
		switch ($_POST['searchWhat'])
		{
			...
			case 'selectLieu':
				...
				else
				{
					...
					{
						?>
						<a href="#" onclick="Changer_Lieu2(<?=$arr['id'];?>, '<?=$arr["nom_technique"];?>', '<?=$_POST['modid'];?>')"><?=$arr["nom_technique"];?></a><br />
						<?php 
					}
				}
				?>
			...
			//Rercherche selon une action de lieu
			case 'selectLieuMenu':
				//$query = 'SELECT lm.*,p.lieuMenuId'
				$query = 'SELECT lm.*'
						. ' FROM ' . DB_PREFIX . 'lieu_menu as lm'
						//. ' LEFT JOIN ' . DB_PREFIX . 'producteur as p ON (p.lieuMenuId = lm.id)'
						. ' WHERE	lm.lieutech=:lieutech'
							. ' AND lm.url=:url;';
				$prep = $db->prepare($query);
				$prep->bindValue(':lieutech',	$_POST['search'],	PDO::PARAM_STR);
				$prep->bindValue(':url',		'Producteur',		PDO::PARAM_STR);
				$prep->execute($db, __FILE__, __LINE__);
				$arrAll = $prep->fetchAll();
				$prep->closeCursor();
				$prep = NULL;
				...
				{
					?>
                    <select name="lieuMenuId" id="lieuMenuId">
                    <?php 
					foreach($arrAll as &$arr)
					{
						?>
                        <option value="<?=$arr["id"];?>"<?php  if(!empty($_POST['modid']) && $_POST['modid'] == $arr["id"]) { echo " selected"; } ?>><?=$arr["caption"];?></option>
						<?php 
					}
					?>
                    </select>
			...
			case 'inventaire_casier':
				$query = 'SELECT id_casier, nom_casier, lieuId'
			...
			case 'inventaire_item':
				$query = 'SELECT db_id, db_nom, db_type, db_desc, db_img'
				...
				else
				{
					foreach($arrAll as &$arr)
					{
                        ...
                        <span class="fakelink" id="item_<?=$arr['db_id'];?>" onmousemove="infobulle(this, event);"><?=$arr['db_type'];?>; <?=$arr['db_nom'];?></span><br />
                        <div id="ib_item_<?=$arr['db_id'];?>" style="width:300px;" class="infobulle">
                            <img src="<?=$SITE_VIRTUAL_PATH;?>../tpl/_common/items/<?=$arr['db_img'];?>" align="left" style="border:0px;margin:3px;" />
                            <?=$arr['db_desc'];?>
                        </div>
						<?php 
					}
			...
			case 'item_asso_perso':
				...
				else
				{
					...
					{
						?>
						<a href="#" onclick="Changer_Perso2('<?=$arr['id'];?>','<?=mysqli_real_escape_string($arr['nom']);?>')"><?=$arr['nom'];?></a><br />
						<?php 
					}
				}
				?>
			...
			case 'item_asso_lieu':
				...
				else
				{
					...
					{
						?>
						<a href="#" onclick="Changer_Lieu2('<?=$arr['nom_technique'];?>','<?=mysqli_real_escape_string($arr['nom_affiche']);?>')"><?=$arr['nom_affiche'];?></a><br />
						<?php 
					}
				}
				?>
			...
			case 'item_asso_boutique':
				$query = 'SELECT id, nom_technique, nom_affiche'
						. ' FROM ' . DB_PREFIX . 'lieu'
						. ' LEFT JOIN ' . DB_PREFIX . 'boutiques_gerants ON id = boutiqueid'
						. ' WHERE	persoid IS NOT NULL'
							. ' AND ('
								. ' id=:id'
								. ' OR nom_technique LIKE :nom'
							. ' )'
