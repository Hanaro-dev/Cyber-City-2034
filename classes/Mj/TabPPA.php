...
//------------------------------------------------------------------------------------------------
		...
		if($mj->accessAdmin())
		{
			//Trouver les PPA associé aux autres MJ
			$query = 'SELECT m.*, p.nom as perso, r.`date` as rep_date, r.mjid as rep_mj'
					. ' FROM ' . DB_PREFIX . 'ppa as m'
					. ' LEFT JOIN ' . DB_PREFIX . 'perso as p ON (p.id = m.persoid)' 
					. ' LEFT JOIN ('
			...
			if(count($arrAll) != 0)
			{
				...
				{
					$PPA_ALL[$i] = $arr;
					$PPA_ALL[$i]['date'] = fctToGameTime($arr['date']);
					...
					{
						//Une réponse du joueur est attendu
						$PPA_ALL[$i]['replyDate'] = 'Att. du joueur';
					}
					else
					{
						//Date de la réponse du joueur
						$PPA_ALL[$i]['replyDate'] = fctToGameTime($arr['rep_date']);
					}
					//mentionner quel MJ a traité le PPA
					//echo "ding : " . $arr['mjid'];
					$query2 = 'SELECT * FROM ' . DB_PREFIX . 'mj'
							. ' WHERE id= ' . $arr['mjid']
							. ' LIMIT 1;';
					$prepa = $db->prepare($query2);
					$prepa->execute($db, __FILE__, __LINE__);
					$arrSel = $prepa->fetchAll();
					$prepa->closeCursor();
					$prepa = NULL;
					
					foreach($arrSel as &$arr2)
					{
						//$PPA_ALL[$i]['replyDate'] .= "<br /><i>Traité par <b>". $arrSel['nom'] ."</b></i>";
						$PPA_ALL[$i]['replyDate'] .= "<br /><i>Traité par <b>". $arr2['nom'] ."</b></i>";
					}
					
					$i++;
				}
			}
			$tpl->set('PPA_ALL', $PPA_ALL);
			$tpl->set('ACCADMIN', 1);
		}
		else
		{
