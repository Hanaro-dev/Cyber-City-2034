...
//                . ' LIMIT :max;';
        ...
        if (count($transactions) > 0) {
            foreach ($transactions as $transaction) {
                try {
                    ...
                    fctBugReport(
                        'Erreur lors d\'une transaction automatique de banque',
    ...
    private static function remisePersoDrogue(&$perso)
    {
        ...
        /*
                 ...
                 * durera deux remises. Est-ce bon ?
                 */
        $query = 'SELECT *'
...
*/
        ...
        foreach ($arrItem as $arr) {
            $perso->changePvMax('-', $arr['db_boost_pv']);
            $perso->changePaMax('-', $arr['db_boost_pa']);

            if ($perso->getPaMax() < $perso->getPa()) {
                $perso->setPa($perso->getPaMax());
            }
            if ($perso->getPvMax() < $perso->getPv()) {
                $perso->setPv('System', 'shock drogue', $perso->getPvMax());
            }

            $valShockPa += $arr['db_shock_pa'];
            $valShockPv += $arr['db_shock_pv'];

            $prepDel->bindValue(':invId', $arr['inv_id'], PDO::PARAM_INT);
            $prepDel->execute($db, __FILE__, __LINE__);

            /* Suppression de ce bout de code qui devrait ne servir à rien dans la mesure où toutes les drogues
            * arrivées ici ne devraient être que des paquets de 1. Pour réactiver, réactivez avec le bloc ligne 382.
        ...
        $perso->changePa('-', $valShockPa);
        $perso->changePv('-', $valShockPv);
        ...
        //        $prepUpd = null;
        /*
        //Remettre le remiseleft à null si il reste des quantités non-consomé
        /(dans le cas ou la drogue est pas effacée)
        $query = 'UPDATE ' . DB_PREFIX . 'item_inv'
                    . ' LEFT JOIN ' . DB_PREFIX . 'item_db ON (db_id=inv_dbid)'
...
*/
    ...
    private static function remisePersoMenotte(&$perso, $expir)
    {
        ...
        foreach ($arrMenotte as &$arr) {
            ...
            Member_He::add('System', $arr['to_id'], 'menotte', '[HJ: Vous n\'avez pas répondu en temps à une demande
            de menottage, elle est donc considérée comme refusée. Merci d\'essayer d\'éviter le plus possible de
            laisser des RP trainer en longueur, car le rythme de jeu s\'en ressent.]');

            if (!empty($arr['inv_persoid'])) {
                Member_He::add('System', $arr['inv_persoid'], 'menotte', '[HJ: Une demande de menottage que vous avez
                faite à expirée, elle est considérée comme refusée.]');
            }
    ...
    private static function remisePersoFouille(&$perso, $expir)
    {
        ...
        foreach ($arrFouille as $arr) {
            if ($arr['reponse'] != 1) {
                //Envoyer un message informant le menotteur que le délais est expiré
                Member_He::add('System', $arr['toid'], 'fouille', '[HJ: Vous n\'avez pas répondu en temps à une
 demande de fouille, elle est donc considérée comme refusée. Merci d\'essayer d\'éviter le plus
 possible de laisser des RP trainer en longueur, car le rythme de jeu s\'en ressent.]');
                Member_He::add('System', $arr['fromid'], 'fouille', '[HJ: Une demande de fouille que vous avez
 faite à expirée, elle est considérée comme refusée.]');
            }
            //Supprimer le menottage
