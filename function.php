...
function BBCodes($in, $domnet=false, $br=true, $img=false)
{
	...
	$find = array(
		...
		/**
			de_brasser = function ()
			{
				var taille = $F('de_taille');
				var reussite = $F('de_reussite');
				var resultat = Math.floor((Math.random()*taille+1));
				
				$('de_resultat').value = resultat;
				$('de_resultat').style.backgroundColor = (resultat>=reussite) ? '#006600' : '#660000';
			}
		**/
...
function fctErrorMSG($msg, $redirectToPage=null, $postParam=null, $ajaxSubmit=true)
{
	...
	{ 
		...
		$src .= "<input type='hidden' name='erreur' value='" . htmlentities($msg, ENT_QUOTES) . "' />";
		if (isset($postParam))
		{
