...
function fctRequire($filePath)
{
    if (!file_exists($filePath)) {
        if (defined('DEBUG_MODE') && DEBUG_MODE) {
            die ('<span style="background-color:#000;color:#FFF;"><strong><font color=red>404:</font></strong> ' .
                 $filePath . '</span>');
        } else {
...
//S'assurer que les caractères francais soient bien pris en comptes.
...
class Main
{
    ...
    */
    ...
    private function getPage()
    {
        ...
        //Page MJ
        if (isset($_GET['mj']) && $this->account->getMjId()!==null) {
    ...
    private function genVisitorPage($file)
    {
        ...
        //Si l'utilisateur viens juste de s'authentifier
        if ($file==='Login2' && $this->session->getVar('logged')===true) {
    ...
    private function genMemberPage($file)
    {
        ...
        if ($this->session->getVar('persoId')===null && !isset($_GET['perso'])) {
            ...
            //Un personnage à été sélection ou est sauvegardé
        } else {
            ...
            try {
                ...
                if ($perso->getHeMsgCount()>15000) {
                    fctBugReport(
                        'Un problème avec le HE à été détecté. Un rapport d\'erreur à été créé et vo
                        tre compte sera vérifié sous peu.',
                        $perso,
                        __FILE__,
                        __LINE__,
                        __FUNCTION__,
...
*/
        $query = 'SELECT topic_title, topic_id
                                    FROM ' . DB_FORUM_PREFIX . 'topics
                                    WHERE forum_id = ' . DB_FORUM_NEWS_ID . '
        ...
        while ($arr = $result->fetch()) {
            $txt .= '<hr class="forumsep" /><div class="forumsujet">';
            $txt .= '<a href="' . FORUM_URL . 'viewtopic.php?f=' . DB_FORUM_NEWS_ID . '&t=' . $arr['topic_id'] . '">';
            $txt .= $arr['topic_title'];
            $txt .= '</a>';
            $txt .= '</div>';

            $arrTopic[] = array('url'=> FORUM_URL . 'viewtopic.php?f=' . DB_FORUM_NEWS_ID . '&t=' . $arr['topic_id']
                                , 'subject'=> $arr['topic_title']);
        }
